...
namespace Oxide.Plugins
{
    ...
    public class IncursionEventGame : RustPlugin
    {
        ...
        public class EventGame
        {
            ...
            public string GetGameStartCriteria()
            {
                ...
                //TODO get this from gamePlayers or something...
                foreach (KeyValuePair<string, EventTeam> team in eventTeams)
                {
                    if (team.Value.teamPlayers.Count < MinPlayersPerTeam)
                    {
                        IemUtils.DLog("too few players on team " + team.Value.TeamName);
                        buf += "<color="+ team.Value.Colour+">"+
                            team.Value.TeamName + "</color>"+
                            "<color=red>(" + team.Value.teamPlayers.Count + "/" +
                               MinPlayersPerTeam + ")</color> ";
                    }
                    else if (team.Value.teamPlayers.Count > MaxPlayersPerTeam)
                    {
                        IemUtils.DLog("too many players on team " + team.Value.TeamName);
                        buf += "<color=" + team.Value.Colour + ">" +
                            team.Value.TeamName + "</color>" +
                        team.Value.TeamName + "<color=red>(" + team.Value.teamPlayers.Count + "/" +
                               MaxPlayersPerTeam + ")</color> ";
                    }
                ...
                {
                    IemUtils.DLog("below min players");
                    buf += "Need total player <color=red>(" + totalPlayers + "/" +
       MinPlayers + ")</color> ";
                }
                ...
                {
                    IemUtils.DLog("above max players");
                    buf += "<color=red>Too many players (" + totalPlayers + "/" +
       MaxPlayers + ")</color> ";
                }
        ...
        public class GameStateManager : IncursionStateManager.StateManager
        {
            ...
            public void CreateGameBanner(string message)
            {
                IemUtils.DLog("caching game banner");
                cachedGameBanner = message;
                foreach (BasePlayer player in BasePlayer.activePlayerList)
                {
                    IncursionUI.CreateGameBanner(player,message);
                }
            }

            public void CreateGameBanner(BasePlayer player, string message)
            {
